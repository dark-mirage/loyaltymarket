...
export function signHs256Jwt(payload: Record<string, unknown>, secret: string) {
...
export function validateTelegramInitDataOrThrow(options: {
  initData: string;
  botToken: string;
  maxAgeSeconds?: number;
}) {
  const { initData, botToken, maxAgeSeconds = 300 } = options;
